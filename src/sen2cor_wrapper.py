...
def sen2cor(scene, resolution, delete_unzipped):
    ...
    # Run sen2cor command
    run_sen2cor = "/Sen2Cor-02.08.00-Linux64/bin/L2A_Process"
    ...
    if os.path.exists(unziped_scene_path):
        ...
        print("The unziped folder {} does not exists, check if unzip is working".format(unziped_scene))
        exit()
...
if __name__ == "__main__":
    ...
    parser.add_argument('input_scene', help="The input scene which may have granules of interest without path")
    parser.add_argument('-r','--resolution', help="The output spatial resolution", default=config.resolution)
    parser.add_argument('-d','--delete_unzipped', help="Delete the unzipped level-1C scene", action="store_true")
